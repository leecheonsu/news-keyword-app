...
def generate_ai_news_analysis(
    ...
    """
    Gemini API를 이용해 기사 제목/요약을 자연스럽게 요약하고
    투자자용 핵심 이슈 3가지를 생성합니다.
    """

    if not GEMINI_API_KEY:
        return {
            "summary": "Gemini API 키가 설정되어 있지 않습니다.",
            "issues": [],
            "investor_note": "missing_api_key"
        }
...
ai_password = st.sidebar.text_input(
    "AI 요약 비밀번호",
    type="password",
    placeholder="허용된 사용자만 입력",
    key="ai_password_input"
)

AI_ACCESS_PASSWORD = st.secrets.get("AI_ACCESS_PASSWORD", "")

ai_allowed = (
    AI_ACCESS_PASSWORD != ""
    and ai_password == AI_ACCESS_PASSWORD
)
...
use_ai_analysis = st.sidebar.checkbox(
    ...
    help="비밀번호가 맞는 사용자만 Gemini API를 사용한 AI 요약을 실행할 수 있습니다.",
    key="use_ai_analysis_checkbox"
)

if not ai_allowed:
    st.sidebar.caption("🔒 AI 요약은 승인된 사용자만 사용할 수 있습니다.")
else:
    st.sidebar.caption("✅ AI 요약 사용 가능")
